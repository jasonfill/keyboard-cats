...
export default function TypingText({ target, cursor, hasError }: Props) {
  return (
    <div className="rounded-2xl bg-white/90 p-5 md:p-7 shadow-inner ring-1 ring-hair">
        ...
        {target.split('').map((ch, i) => {
          ...
          if (isCurrent) {
            cls = hasError
              ...
              : 'bg-accent text-white rounded ring-2 ring-edge'
          }
          return (
            <span
              key={i}
              className={`${cls} ${ch === ' ' && !isCurrent ? 'text-faint' : ''} px-0.5`}
            >
              {display}
            </span>
          )
